...
const codeBlockRenderer: RendererExtension = {
  ...
  renderer(token) {
    ...
    const blockIndex = codeBlockRenderIndex++
    const escapedLang = escapeHtml(lang)
    const langText = escapedLang || 'plaintext'
    const langLabel = `<button class="code-lang-badge preview-code-lang-badge" data-code-block-index="${blockIndex}" data-language="${escapedLang}" type="button">
          <span class="code-lang-label">${langText}</span>
          <span class="code-lang-chevron">▾</span>
        </button>`
    ...
    return `<div class="code-block-wrapper" data-code-block-index="${blockIndex}">
