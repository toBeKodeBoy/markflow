...
describe('首页留白', () => {
  ...
  it('首页应为 Logo + 命令列表，避免所有区块共用 12px 缝', () => {
    ...
    expect(home).toMatch(/data-testid="empty-home-cluster"/)
  ...
  it('空态画布铺满区域 2，Logo 与命令作为一组垂直居中', () => {
    expect(css).toMatch(/\.app\s*\{[^}]*height:\s*100%/)
    expect(css).not.toMatch(/\.app\s*\{[^}]*height:\s*100vh/)
    ...
    expect(css).toMatch(/\.empty-tabs-state\s*\{[^}]*display:\s*flex/)
    expect(css).toMatch(/\.empty-tabs-state\s*\{[^}]*flex-direction:\s*column/)
    expect(css).toMatch(/\.empty-tabs-state\s*\{[^}]*align-items:\s*center/)
    expect(css).toMatch(/\.empty-tabs-state\s*\{[^}]*justify-content:\s*center/)
    expect(css).not.toMatch(/\.empty-tabs-state\s*\{[^}]*safe\s+center/)
    expect(css).not.toMatch(/\.empty-tabs-state\s*\{[^}]*radial-gradient/)
    expect(css).toMatch(/\.empty-home-cluster\s*\{[^}]*display:\s*flex/)
    expect(css).toMatch(/\.empty-home-cluster\s*\{[^}]*flex-direction:\s*column/)
    expect(css).toMatch(/\.empty-home-cluster\s*\{[^}]*align-items:\s*center/)
    expect(css).toMatch(/\.empty-home-cluster\s*\{[^}]*gap:\s*16px/)
    expect(css).not.toMatch(/\.empty-home-cluster\s*\{[^}]*flex:\s*1/)
    expect(css).not.toMatch(/\.empty-home-cluster\s*\{[^}]*min-height:\s*100%/)
    expect(css).toMatch(/\.empty-home-mark\s*\{[^}]*width:\s*160px/)
    expect(css).toMatch(/\.empty-home-mark\s*\{[^}]*height:\s*160px/)
    expect(css).toMatch(/\.empty-home-mark\s*\{[^}]*border-radius:\s*24px/)
    expect(css).toMatch(/\.empty-home-mark\s*\{[^}]*font-size:\s*96px/)
    expect(css).not.toMatch(/\.empty-home-brand\s*\{[^}]*position:\s*absolute/)
    expect(css).not.toMatch(/\.empty-home-commands\s*\{[^}]*position:\s*absolute/)
    expect(css).not.toMatch(/top:\s*calc\(\s*50%\s*\+\s*172px\s*\)/)
  ...
  it('三行命令应为 flex 横排，文案定宽、快捷键不换行', () => {
    expect(css).toMatch(/\.empty-home-commands\s*\{[^}]*display:\s*flex/)
    expect(css).toMatch(/\.empty-home-commands\s*\{[^}]*flex-direction:\s*column/)
    expect(css).toMatch(/\.empty-home-commands\s*\{[^}]*gap:\s*2px/)
    expect(css).toMatch(/\.empty-home-commands\s*\{[^}]*width:\s*max-content/)
    expect(css).not.toMatch(/\.empty-home-commands\s*\{[^}]*min-width:\s*320px/)
    ...
    expect(css).toMatch(/\.empty-home-command\s*\{[^}]*align-items:\s*center/)
    expect(css).toMatch(/\.empty-home-command\s*\{[^}]*gap:\s*12px/)
    expect(css).toMatch(/\.empty-home-command\s*\{[^}]*background:\s*transparent/)
    expect(css).toMatch(/\.empty-home-command\s*\{[^}]*white-space:\s*nowrap/)
    expect(css).toMatch(/\.empty-home-command\s*\{[^}]*padding:\s*4px\s+2px/)
    expect(css).toMatch(/\.empty-home-command\s*\{[^}]*color:\s*var\(--empty-home-fg\)/)
    expect(css).toMatch(/\.empty-home-command-label\s*\{[^}]*flex:\s*0\s+0\s+4em/)
    expect(css).toMatch(/\.empty-home-command-keys\s*\{[^}]*flex-shrink:\s*0/)
    expect(css).toMatch(/\.empty-home-command\s*\{[^}]*transition:\s*none/)
    expect(home).toMatch(/class="empty-home-command-label"/)
  })

  it('keycap 应为小圆角灰底，加号与按键留缝', () => {
    expect(css).toMatch(/:root\s*\{[^}]*--empty-home-fg:\s*#444444/)
    expect(css).toMatch(/:root\s*\{[^}]*--empty-home-kbd-bg:\s*#f3f3f3/)
    expect(css).toMatch(/:root\s*\{[^}]*--empty-home-kbd-border:\s*#d4d4d4/)
    expect(css).toMatch(/\[data-theme="dark"\]\s*\{[^}]*--empty-home-fg:\s*#bbbbbb/)
    expect(css).toMatch(/\[data-theme="dark"\]\s*\{[^}]*--empty-home-kbd-bg:\s*#2d2d3a/)
    expect(css).toMatch(/\[data-theme="dark"\]\s*\{[^}]*--empty-home-kbd-border:\s*#45455a/)
    expect(home).toMatch(/class="empty-home-key-sep"/)
    expect(css).toMatch(/\.empty-home-command-keys\s*\{[^}]*gap:\s*0/)
    expect(css).toMatch(/\.empty-home-key-sep\s*\{[^}]*margin:\s*0\s+5px/)
    expect(css).not.toMatch(/\.empty-home-command-keys\s*>\s*span\s*\{/)
    expect(css).toMatch(/\.empty-home-command kbd\s*\{[^}]*border-radius:\s*3px/)
    expect(css).toMatch(/\.empty-home-command kbd\s*\{[^}]*font-size:\s*11px/)
    expect(css).toMatch(/\.empty-home-command kbd\s*\{[^}]*font-weight:\s*400/)
    expect(css).toMatch(/\.empty-home-command kbd\s*\{[^}]*background:\s*var\(--empty-home-kbd-bg\)/)
    expect(css).toMatch(/\.empty-home-command kbd\s*\{[^}]*border:\s*1px solid var\(--empty-home-kbd-border\)/)
    expect(css).toMatch(/\.empty-home-command kbd\s*\{[^}]*color:\s*var\(--empty-home-fg\)/)
    expect(css).not.toMatch(/\.empty-home-command kbd\s*\{[^}]*border-radius:\s*999px/)
    expect(css).toMatch(/\.empty-home-command kbd\s*\{[^}]*transition:\s*none/)
  })
