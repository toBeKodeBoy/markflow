...
- 关联文档：
  ...
  - [`暗色模式滚动条适配技术方案.md`](./暗色模式滚动条适配技术方案.md) / [`../prds/暗色模式滚动条适配.md`](../prds/暗色模式滚动条适配.md)（暗色滚动条适配，待实施；不占用 Phase B）
...
| L6 | 暗色滚动条破相 | WYSIWYG / 源码露出 Windows 白轨道；已单独立项，见 scrollbar PRD / 技术方案 |
...
- 字体与微动效（L3，低优）
- 暗色滚动条已拆出独立 PRD / 技术方案（L6），不在本队列 Phase 内实施
...
| `暗色模式滚动条适配技术方案.md` + `docs/prds/暗色模式滚动条适配.md` | 暗色滚动条独立立项；本文只做索引，不在此展开实现 |
