...
- **顶栏搜索入口**：与 `Ctrl+K` 共用 `SearchModal` 开关

### Changed

- 主题切换收敛为设置面板唯一入口；移除顶栏明暗快捷按钮，避免与「跟随系统」语义冲突
- 设置页 PDF 文案改为指引「文件 → 导出 PDF」
- 搜索弹窗与置顶标记改用 `AppIcon`（去掉 emoji）
- README / 样式清理：移除已下线标签系统相关描述与死 CSS
